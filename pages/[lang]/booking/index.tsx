...
const BookingPage = ({
  ...
  const [_filterRooms, { data: filteredRooms }] = useLazyQuery(
    ROOMS_AGGREGATE,
    {
      onCompleted() {
  ...
  */
  ...
  const pickRoomHandler = (room: any) => {
    console.log("pickRoomHandler", room);
    let pickedRoom = {
    ...
    if (filterValues?.roomDetails.length === 1) {
      ...
      console.log("will setSelectedRooms", [...newSelectedRooms]);
      setSelectedRooms([...newSelectedRooms]);
    }
    setStepTitle({
      ar: "اختر السعر ",
